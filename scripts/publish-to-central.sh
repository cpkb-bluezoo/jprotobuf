...
#
# Optional:
#   VERSION                  - the release version, e.g. 1.0.0 - if unset,
...
# jprotobuf-$VERSION.pom) - if its own <version> disagrees with $VERSION,
...
BUNDLE_DIR="$WORKDIR/bundle/org/bluezoo/jprotobuf/$VERSION"
mkdir -p "$BUNDLE_DIR"

cp pom.xml "$BUNDLE_DIR/jprotobuf-$VERSION.pom"
cp "dist/jprotobuf-$VERSION.jar" "$BUNDLE_DIR/"
cp "dist/jprotobuf-$VERSION-sources.jar" "$BUNDLE_DIR/"
cp "dist/jprotobuf-$VERSION-javadoc.jar" "$BUNDLE_DIR/"
